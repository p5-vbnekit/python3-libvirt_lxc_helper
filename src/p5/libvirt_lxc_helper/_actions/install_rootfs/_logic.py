...
def _private():
    ...
    def _make_thread_asynchronizer():
        _loop = asyncio.get_running_loop()
        assert isinstance(_loop, asyncio.AbstractEventLoop)
        with concurrent.futures.ThreadPoolExecutor() as _pool:
            yield _make_asynchronizer(delegate = lambda delegate: _loop.run_in_executor(_pool, delegate))
    ...
    async def _coroutine(dry: bool, source: typing.Optional[str], destination: str):
        ...
        with _make_thread_asynchronizer() as _asynchronizer:
            async with _connection_module.make(domain = destination, asynchronizer = _asynchronizer) as _connection:
